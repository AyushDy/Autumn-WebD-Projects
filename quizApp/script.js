...
const startPageContainer = document.querySelector('.starter');
const startPage = document.querySelector('.start-page');
const configurePage =  document.querySelector('.configure')
const mainContainer = document.querySelector('.main-container');
const questionContainer = document.querySelector('.question-container');

const noOfQuesField = document.querySelector('#no-of-questions');
const difficultyField = document.querySelector('#difficulty');
const selectButton = document.querySelector('#select');

const question = document.querySelector('#question');
const optionsContainer = document.querySelector('.options');
const difficultyContainer = document.querySelector('#diff');
...
startButton.addEventListener('click',renderConfigurePage);
selectButton.addEventListener('click',renderQuestionsPage);
nextButton.addEventListener('click',displayNextQuestion);



function checkAnswer(event){
    optionsContainer.classList.add('disabled-clicks');
...
function displayNextQuestion(){
    ...
    attemptedHistory.push(obj);
    currentQuesion = currentQuesion + 1;
    if(currentQuesion >= noOfQuestions) {
        displayResult();
        return;
    }
    startQuiz();
...
function displayResult(){
    ...
    attemptedHistory.forEach(obj =>{
        const quesDiv = document.createElement('div');
        const h4 = document.createElement('h4');
        h4.innerText = obj.question;
        const p= document.createElement('p');
        p.innerText = `Correct :`;
        const p2= document.createElement('p');
        p2.innerText = `Selected :`;

        const correctDiv = document.createElement('div');
        correctDiv.innerText = obj.correct;
        correctDiv.classList.add('opt','correct','disabled-clicks');

        const selectedDiv = document.createElement('div');
        selectedDiv.innerText = obj.selected;
        selectedDiv.classList.add('opt','disabled-clicks')

        if(obj.selected===obj.correct){
            selectedDiv.classList.add('correct');
        }else if(obj.selected!='Not Attempted'){
            selectedDiv.classList.add('wrong');
        }

        quesDiv.appendChild(h4);
        quesDiv.appendChild(p);
        quesDiv.appendChild(correctDiv);
        quesDiv.appendChild(p2);
        quesDiv.appendChild(selectedDiv);
...
function renderQuestionsPage(){
    noOfQuestions  = parseInt(noOfQuesField.value);
    difficulty = difficultyField.value;

    questionsArray = getQuestions(noOfQuestions,difficulty);
...
function startQuiz(){
    timerSeconds = setInterval(displayTimer, 1000);
    optionsContainer.classList.remove('disabled-clicks');
    const questionToDisplay = questionsArray[currentQuesion];
    question.innerText = questionToDisplay.question;

    optionsContainer.innerHTML = '';
    questionToDisplay.options.forEach(opt=>{
        optionsContainer.appendChild(createOptionDiv(opt));
    })

    correct = questionToDisplay.correct;
    questionText = questionToDisplay.question;
    selected = 'Not Attempted';

    difficultyContainer.className = questionToDisplay.difficulty;
    difficultyContainer.innerText = questionToDisplay.difficulty;

    totalSeconds = timers[questionToDisplay.difficulty];
    seconds = totalSeconds;

    const progressDiv = infoContainer.querySelector('p');
    progressDiv.innerText = `Question ${currentQuesion+1} of ${noOfQuestions}`;
}


function displayTimer(){
    let secs= seconds/1000;
    document.querySelector('.time-left').innerText = padZero(secs);
    const percentage = ((totalSeconds-seconds)/totalSeconds)*100;
...
function getQuestions(size, diff){
    let questionArray =  [];

    if(diff === 'Easy'){
        questionArray = getRandomQuestions(size,easyJavaScriptQuestions);
    }else if(diff === 'Medium'){
        questionArray = getRandomQuestions(size,mediumJavaScriptQuestions);
    }else if(diff === 'Hard'){
        questionArray = getRandomQuestions(size, hardJavaScriptQuestions);
    }else if(diff === 'Mixed'){
        let size1 = Math.floor(size/3)
        let size2 = Math.floor(size/3)
        let size3=  size- size1 -size2;

        const easy =  getRandomQuestions(size1, easyJavaScriptQuestions);
        const medium = getRandomQuestions(size2, mediumJavaScriptQuestions);
        const hard =  getRandomQuestions(size3, hardJavaScriptQuestions);

        questionArray=[...easy,...medium,...hard];
    }else{
...
function getRandomQuestions(size, Array){
    const shuffledEasy  = shuffle(Array);
    return shuffledEasy.slice(0, size);
}
...
function renderConfigurePage(){
    for(let i=5;i<=30;i++){
        noOfQuesField.appendChild(createOption(i));
    }
    difficultyField.appendChild(createOption('Easy'));
    difficultyField.appendChild(createOption('Medium'));
    difficultyField.appendChild(createOption('Hard'));
    difficultyField.appendChild(createOption('Mixed'));

    startPage.style.display = 'none';
    configurePage.style.display = 'flex';
}
